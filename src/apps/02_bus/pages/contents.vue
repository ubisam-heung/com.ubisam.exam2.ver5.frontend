<template>
  <v-app id="inspire">
    <v-app-bar app>
      ...
      <v-toolbar-title>{{ $t("frontend.index.title") }} {{ subtitle }} </v-toolbar-title>
    ...
    <v-navigation-drawer permanent v-model="drawer">
      <v-list nav>
        ...
        <v-list-item to="/contents/repairs" v-if="isAdmin"> {{ $t("frontend.contents.repairs.title") }} </v-list-item>
        <v-list-item to="/contents/sessions" v-if="isAdmin"> {{ $t("frontend.contents.sessions.title") }} </v-list-item>
        <v-divider></v-divider>
...
export default {
  ...
  mounted() {
    ...
    $busServer.oauth2.userinfo("ROLE_ADMIN")
      .then((r) => {
